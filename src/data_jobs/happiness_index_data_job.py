...
class HappinessIndex(Job):
    def __init__(self, job_name):
        ...
        self.spark = spark_utils.SparkUtils().get_spark_session(
            "happiness_index_data_job"
        )
    ...
    def run(self):
        df_csv = self.utils.read_data(
            self.spark, self.happiness_data, "csv", happiness_data_schema
        )
        ...
        df4.coalesce(1).write.format("csv").mode("overwrite").option(
            "header", True
        ).option("sep", ",").save(self.happiness_data_target)
    ...
    def correlation_transform(self, df: DataFrame):
        ...
        return (
            df.withColumn(
                "c_happiness_life_exp",
                lit(
                    round(
                        df.corr("happiness_rank", "health_life_expectancy"), 2
                    )
                ),
            )
            .withColumn(
                "c_happiness_generosity",
                lit(round(df.corr("happiness_rank", "generosity"), 2)),
            )
            .withColumn(
                "c_happiness_corruption",
                lit(
                    round(
                        df.corr(
                            "happiness_rank", "trust_government_corruption"
                        ),
                        2,
                    )
                ),
            ...
            .withColumn(
                "c_happiness_freedom",
                lit(round(df.corr("happiness_rank", "freedom"), 2)),
            )
            .withColumn(
                "c_happiness_economy",
                lit(
                    round(
                        df.corr("happiness_rank", "economy_gdp_per_capita"), 2
                    )
                ),
            )
        )
    ...
    def generate_annotation(self, df: DataFrame):
        self.logger.info(
            f"Generating annotation columns for Happiness Index data"
        )
        return df.withColumn(
            "annotations",
            when(col("happiness_rank") <= 5, "Doing Well")
